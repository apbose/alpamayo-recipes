...
readonly BASE_CHECKPOINT="${BASE_CHECKPOINT:-/home/scratch.abose_sw/alpamayo-assets/checkpoints/Alpamayo-1.5-10B-A1-format}"
...
readonly MAX_STEPS="${MAX_STEPS:-1986}"
readonly SAVE_STRATEGY="${SAVE_STRATEGY:-steps}"
readonly SAVE_STEPS="${SAVE_STEPS:-662}"
readonly CONFIG_NAME="sft_stage2_trajectory_shortcut_hf_10to5_reference_ema"
readonly RUN_NAME="${RUN_NAME:-alpamayo15_hf300gb_10to5_reference_ema_5295clips_3epochs_20260919_r1}"
...
if [[ ! "$MAX_STEPS" =~ ^[1-9][0-9]*$ ]]; then
  printf 'MAX_STEPS must be a positive integer.\n' >&2
  exit 2
fi
...
flow_step_size=0.1
shortcut_step_size=0.2
inference_target_steps=5
loss_estimator=reference_partition
bootstrap_every=4
global_allocation=6_flow_2_shortcut
teacher=ema
ema_decay=0.999
