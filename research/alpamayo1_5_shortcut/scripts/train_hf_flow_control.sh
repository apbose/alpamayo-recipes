...
readonly FLOW_RUN_NAME="${FLOW_RUN_NAME:-alpamayo15_hf300gb_flow_control_1epoch}"
readonly RUN_ROOT="${RUN_ROOT:-/home/scratch.abose_sw/alpamayo-assets/runs/$FLOW_RUN_NAME}"
...
mkdir -p "$OUTPUT_DIR" "/tmp/alpamayo15-$FLOW_RUN_NAME-triton"
printf 'status=running\nstarted_utc=%s\nrun_name=%s\nnproc_per_node=%s\nmax_steps=%s\nsource_coverage_gb=301.7651036\ntraining_rows=5295\ndataloader_workers_per_rank=%s\nloss_objective=flow_matching_only\nstep_size_adapter=false\nroute_conditioning=false\n' \
  "$(date -u +%FT%TZ)" "$FLOW_RUN_NAME" "$NPROC_PER_NODE" \
  "${MAX_STEPS:-epoch}" "$DATALOADER_WORKERS" > "$STATUS_FILE"
...
export TRITON_CACHE_DIR="/tmp/alpamayo15-$FLOW_RUN_NAME-triton"
...
"$TORCHRUN_BIN" --standalone --nproc_per_node="$NPROC_PER_NODE" \
  ...
  --config-name sft_stage2_trajectory_hf_flow_control \
